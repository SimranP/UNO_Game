...
app.get('/provideIndexForm',function(req,res,next){
	...
	if(all_games.length > 0){
		...
		waiting_games.forEach(function(game){
			joinForm+= '<input type="text"  name="joinName" placeholder="Enter your name" id ='+game.id.split(":")[1]+' ></input><button name = "join" id = "join" value='+ game.id+'> join </button> &nbsp;&nbsp;&nbsp; '+game.getplayersLimit()+' <br> This game is going on between :<br>'+game.getPlayerNames().join('<br>')+' <br><hr><br>'
		});
		content.joinForm = joinForm;
	}
		content.indexForm = '<input type="text" name="name" placeholder="Enter your name" id="name"/><input type="number" name="noOfPlayers" placeholder="no.between 1-8" id="noOfPlayer" /> players.<button name="join" id="loadGame"> Load Game </button>';
	res.send(content);
});
