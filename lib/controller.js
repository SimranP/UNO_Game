...
app.get('/provideIndexForm',function(req,res,next){
	...
	if(all_games.length > 0){
		...
		waiting_games.forEach(function(game){
			joinForm+= 'Enter your name : <input type="text"  name="joinName" id ='+game.id.split(":")[1]+' ></input><button name = "join" id = "join" value='+ game.id+'> join </button><br><hr><br>'
		});
		content.joinForm = joinForm;
	}
		content.indexForm = 'Enter your name : <input type="text" name="name" id="name"/>Play with :<input type="number" name="noOfPlayers" id="noOfPlayer" /> players.<button name="join" id="loadGame"> Load Game </button>';
	res.send(content);
});
