...
app.get('/login',function(req,res){
  var playerName
    (req.user) && (playerName = req.user.displayName)||(playerName = req.url.split("=")[1]);
  	if(!req.cookies.name) {
      if(req.user)
        var value = playerName+":"+req.user.id;
      else
  		  var value  = playerName+":undefined";
...
app.post('/createNewGame',function(req,res,next){
  ...
  var socialNetworkId = req.cookies.name.split(':')[1];
	var player = new Player(playerName,id,socialNetworkId);
...
var getPlayerDetails = function(player){
  ...
  dataOfPlayerToReturn["socialNetworkId"] = player.socialNetworkId ;
