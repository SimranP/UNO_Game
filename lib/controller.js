...
app.get('/provideIndexForm',function(req,res,next){
	...
	if(all_games.length > 0){
		...
		waiting_games.forEach(function(game){
			joinForm+= '<input type="text"  name="joinName" placeholder="Enter your name" id ='+game.id.split(":")[1]+' ></input><br><button name = "join" id = "join" value='+ game.id+'> join </button><div id="playerDetails"> '+game.getplayersLimit()+' <br> This game is going on between :<br>	'+game.getPlayerNames().join('<br>')+'</div> <br><hr>'
		});
		content.joinForm = joinForm;
	}
		content.indexForm = '<input type="text" name="name" placeholder="Enter your name" id="name"/><br><input type="number" name="noOfPlayers" placeholder="no. of players between 1-8" id="noOfPlayer" /> <br><button name="join" id="loadGame"> Start Game </button><hr>';
	res.send(content);
});
